...
#					# this filter cuts out items that have a small Volume
            ...
            average_volume_dict[s] = int(str(items[5]).strip('\"'))
...
#	sorted_json_output = sorted(json_output, key=lambda item: abs(float(item['sigma_over_p_0'])), reverse=True)
    ...
    body = '\t'.join(['Symbol', 'Name', 'Latest Date', 'Trade Time', 'Sig/P', 'Price', 'Volume', 'RSI', 'SMA', 'Sigma', 'SL Offset', 'Vol Pctle', 'Abs Vol', 'Sig Span', 'Hist Sig']) + '\n\n'
    for item in sorted_output:
...
def convert_to_dict(result):
    header_names = ('symbol', 'latest_name', 'latest_date', 'latest_time', 'latest_price', 'volume', 'sigma_over_p_0', 'rsi_0', 'sma_0', 'sigma_0', 'stop_loss_offset', 'volatility_percentile', 'volatility_0', 'sigma_span_0', 'historical_sigma')
...
#		pass
    ...
    ### Liquidity Check
    if volume < 0:
        return False, "volume not high enough for " + symbol
    market_cap = volume * latest_price
    if market_cap < 0:
        return False, "market cap is too low for " + symbol
    ...
    stop_loss_offset = round(0.3 * sigma_0, 4)

    sigma_span, historical_sigma = tools.sigma_span(merged_prices, 5, 100, sigma_input = None)
    sigma_span_0 = round(sigma_span[-1], 4)
    sigma_span_1 = sigma_span[-2]
    historical_sigma_0 = round(historical_sigma[-1], 4)

    # print "Symbol: ", symbol, merged_prices[-5:], rsi_0, rsi_1, sma_0, sigma_0, sigma_over_p_0


    ret_values =  (symbol, latest_name, latest_date, latest_time, sigma_over_p_0, latest_price, volume, rsi_0, sma_0, sigma_0, stop_loss_offset, volatility_percentile, volatility_0, sigma_span_0, historical_sigma_0)

    if symbol in in_trade:
        return True, ret_values

    if latest_price > sma_0:
        if sigma_span_1 > -1.6 and sigma_span_0 < -1.6:
            return True, ret_values
