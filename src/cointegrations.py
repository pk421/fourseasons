...
def get_new_exit_parms(price_window, starting_synth_price, stop_loss, profit_target):
    ...
    abs_stop_loss = 0.06 * starting_synth_price

    trading_up_stop_loss = (starting_synth_price - min((stop_loss * sigma_price), abs_stop_loss))
    trading_down_stop_loss = (starting_synth_price + min((stop_loss * sigma_price), abs_stop_loss))
